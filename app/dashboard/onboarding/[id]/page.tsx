          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
            ...
            <div className="flex flex-col gap-3 mb-6">
              <textarea 
                ...
                className="w-full bg-black/20 border border-white/10 rounded-xl py-3 px-4 text-white focus:outline-none focus:border-cyan-500 min-h-[100px] resize-y text-sm"
              />
              <div className="flex justify-end">
                <button 
                  onClick={addNote} 
                  disabled={isProcessing || !newNote.trim()} 
                  className="px-6 py-2 bg-cyan-500 hover:bg-cyan-400 disabled:opacity-50 disabled:cursor-not-allowed text-slate-950 font-bold rounded-xl transition-colors text-sm shadow-[0_0_15px_rgba(34,211,238,0.2)]"
                >
                  Add Note
                </button>
              </div>
            ...
            <div className="space-y-4">
              {notes.length === 0 ? (
                <p className="text-slate-500 text-sm text-center py-4">No internal notes yet.</p>
              ) : (
                notes.map(note => (
                  <div key={note.id} className="pb-4 border-b border-white/10 last:border-0 last:pb-0">
                    <p className="text-slate-300 text-sm mb-2 whitespace-pre-wrap break-words leading-relaxed">{note.note_text}</p>
