    ...
    <div className="space-y-6 max-w-6xl mx-auto">
        ...
        <form onSubmit={handleAddClient} className="glass-card p-6 border-violet-500/30 animate-fade-in space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2 flex items-center gap-2 pt-2">
              <input type="checkbox" id="alsoBook" checked={alsoBookAppointment} onChange={e => setAlsoBookAppointment(e.target.checked)} className="rounded border-white/10 bg-black/40 text-violet-500 focus:ring-violet-500" />
      ...
      <div className="glass-card overflow-hidden">
        <div className="p-4 border-b border-white/5 flex items-center justify-between gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search clients by name, email, phone..." className="w-full bg-black/20 border border-white/10 rounded-lg py-2 pl-10 pr-4 text-sm text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-violet-500/50" />
          </div>
          <span className="text-xs text-zinc-500">{filteredClients.length} {filteredClients.length === 1 ? 'client' : 'clients'} found</span>
        </div>
        {loading ? (
            ...
            {filteredClients.map(client => (
              <div key={client.id} className="p-4 hover:bg-white/[0.02] transition-colors flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  <button
                    onClick={() => { setBookingClient({ name: client.name, phone: client.phone || '' }); setIsAppointmentModalOpen(true); }}
      ...
      {tenantId && (
        <NewAppointmentForm
          ...
          onClose={() => { setIsAppointmentModalOpen(false); setBookingClient(null); }}
          onSuccess={() => {}}
